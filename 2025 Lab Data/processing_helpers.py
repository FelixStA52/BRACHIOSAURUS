...
def identify_peaks(amps, freqs, noise_threshold=0, tol=TOL, min_freq=MIN_FREQ):
    ...
    for i in range(1, len(amps)-1):
        ...
        if amps[i] > amps[i-1] and amps[i] > amps[i+1]:
            ...
            if np.any(noise_mask):
                ...
                local_noise = np.mean(amps[noise_mask])
                local_noise_std = np.std(amps[noise_mask])
                # Only keep peak if significantly above local noise
                if amps[i] > local_noise + LOCAL_NOISE_THRESH*local_noise_std:
    ...
    if fundamental1 is not None:
        # half‑width of the window:
        delta_f = fundamental1 * (1 + L_TOL)
        
        for freq, amp in peaks:
            # Skip f1 itself
            if abs(freq - fundamental1) < tol * fundamental1:
                continue
                
            # Skip harmonics of f1 (n=2 to 3)
            skip_harmonic = False
            for n in range(2, 4):
                harmonic_freq = n * fundamental1
                if abs(freq - harmonic_freq) < tol * harmonic_freq:
                    skip_harmonic = True
                    break
            if skip_harmonic:
                continue
                
            # Build absolute-frequency mask over the full spectrum
            low = freq - delta_f
            high = freq + delta_f
            ...
            # Only keep candidate if it is the local maximum
    ...
    for freq, amp in peaks:
        # Skip fundamentals (using tolerance)
        if (fundamental1 and abs(freq - fundamental1) < tol * fundamental1) or \
           (fundamental2 and abs(freq - fundamental2) < tol * fundamental2):
            continue
        
        assigned = False
        
        # Check if harmonic of fundamental1 (n=2 to 3)
        if fundamental1:
            for n in range(2, 4):
                harmonic_freq = n * fundamental1
                if abs(freq - harmonic_freq) < tol * harmonic_freq:
        ...
        # Check if harmonic of fundamental2 (n=2 to 3)
        if not assigned and fundamental2:
            for n in range(2, 4):
                harmonic_freq = n * fundamental2
                if abs(freq - harmonic_freq) < tol * harmonic_freq:
                    result['fundamental2']['harmonics'].append(freq)
                    assigned = True
                    break
        
        # Check if modulated peak (f2 ± n*f1, n=1)
        if not assigned and fundamental1 and fundamental2:
            for n in range(1, 2):
        ...
        # If none of the above, mark as other peak
        if not assigned:
        ...
        for j in range(i+1, f1_len):
            ...
            for n in [1,2]:
                ...
                if abs(ratio/n - 1) < tol:
                    suggest = main[i] / n
                    if suggest < f1_arr[i,0]:
                        f1_arr[i,0] = suggest
                    f1_arr[i,1] += 1
                    f1_arr[j,1] += 1
                elif abs(ratio*n - 1) < tol:
                    suggest = main[j] / n
                    if suggest < f1_arr[j,0]:
                        f1_arr[j,0] = suggest
                    f1_arr[i,1] += 1
                    f1_arr[j,1] += 1
        if f1_arr[i,0] == 0:
...
def frequency_analysis(freq1, freq1_harmonic=[], freq2=[], freq2_harmonic=[], freq2_modulated=[], others=[], tol=0.05):
    ...
    filtered_freq1 = check_multiple(all_freq1, others=freq2, tol=tol)
...
def freq_to_len(f1, f2, r_outer=R_O, r_inner=R_I, young=YOUNG, rho=DENSITY, m_tip=MASS, initial_guess=POLE_MAX_L):
    ...
    len1 = np.array([sp.fsolve(lambda L: freq_estimator(L,young,r_outer,r_inner,rho,m_tip) - freq, x0=initial_guess1)[0] for freq in f1])
