...
if data_processing:
    ...
    for u in range(np.shape(box_n)[0]):
        ...
        #processed_data.csv: time, temperature, motion length, error, data length, error, depth sensor length, error, F1, error, F2, error, L1, error, L2, error
        ...
        total_col_final_data = 16  # Updated: was 12, now 16 to accommodate L1, L1_err, L2, L2_err
        final_data = np.zeros((len(times), total_col_final_data)) #for csv file: time, temperature, motion length, error, data length, error, depth sensor length, error, F1, error, F2, error, L1, error, L2, error
        ...
        for time in times:
            ...
            for i in range(0, np.shape(motion)[0]):
                ...
                peak_info = identify_peaks(amp_fft, pos_freq, noise_threshold=3*avg_amp)
            ...
            err_comp = np.std(filtered_all_len)/np.sqrt(np.shape(filtered_all_len)[0])
            
            data_depth_col = 6
            cm_to_m = 100
            
            #for csv file: time, temperature, motion length, error, data length, error, depth sensor length, error, F1, error, F2, error, L1, error, L2, error
            final_data[final_data[:,time_col]==time,temp_col] = np.mean(data[:,data_temp_col]) + temp_offset
        ...
        header="Time,Temperature,Motion Length,Error,Data Length,Error,Depth Sensor Length,Error,F1,Error,F2,Error,L1,Error,L2,Error"  # Updated header
