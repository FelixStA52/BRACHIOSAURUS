...
RTC_DATA_ATTR String box_n = BOX_NUMBER; //number of the box
...
RTC_DATA_ATTR int loopsPerWakeUp = LOOPS_PER_WAKE_UP; //how many data points are taken in a wake up loop

RTC_DATA_ATTR long int secToSleep = SEC_TO_SLEEP; //time (seconds) the microcontroller goes to sleep between taking data

RTC_DATA_ATTR long int wifiTimeSleep = DEPLOYMENT_WAIT;

RTC_DATA_ATTR long int deploy_start = DEPLOYMENT_START;

RTC_DATA_ATTR long int deploy_end = DEPLOYMENT_END;
...
RTC_DATA_ATTR int wifi_counter = 0; //counter for times it went into deep sleep
...
void setup()
{
  ...
  if (counter == 0 && wifi_counter == 0) {
    power_on();
    loadParameters(); // Try to load from SD first
  ...
  if (epochTime >= deploy_start && epochTime < deploy_end) {
  ...
  int sleepTime = max(min(timeToData, timeToWiFi), 1);
...
void main_routine(){
  ...
  codeEnd = millis();
  epochTime += (codeEnd - codeStart) / 1000 + secToSleep;
  codeStart = codeEnd;
  saveParameters();
}
...
void saveParameters() {
  if (SD.begin(CS_PIN)) {
    File file = SD.open("/parameters.txt", FILE_WRITE);
    if (file) {
      file.println("epochTime=" + String(epochTime));
      file.println("box_n=" + box_n);
      file.println("loopsPerWakeUp=" + String(loopsPerWakeUp));
      file.println("secToSleep=" + String(secToSleep));
      file.println("wifiTimeSleep=" + String(wifiTimeSleep));
      file.println("deploy_start=" + String(deploy_start));
      file.println("deploy_end=" + String(deploy_end));
      file.println("counter=" + String(counter));
      file.println("nextDataTime=" + String(nextDataTime));
      file.println("nextWiFiCheckTime=" + String(nextWiFiCheckTime));
      file.println("wifi_accessed=" + String(wifi_accessed));
      file.close();
    }
  }
}
...
void loadParameters() {
  if (SD.begin(CS_PIN) && SD.exists("/parameters.txt")) {
    File file = SD.open("/parameters.txt", FILE_READ);
    while (file.available()) {
      ...
      if (eqIndex > 0) {
        ...
        if (key == "epochTime") epochTime = value.toInt();
        else if (key == "box_n") box_n = value;
        else if (key == "loopsPerWakeUp") loopsPerWakeUp = value.toInt();
        else if (key == "secToSleep") secToSleep = value.toInt();
        else if (key == "wifiTimeSleep") wifiTimeSleep = value.toInt();
        else if (key == "deploy_start") deploy_start = value.toInt();
        else if (key == "deploy_end") deploy_end = value.toInt();
        else if (key == "counter") counter = value.toInt();
        else if (key == "nextDataTime") nextDataTime = value.toInt();
        else if (key == "nextWiFiCheckTime") nextWiFiCheckTime = value.toInt();
        else if (key == "wifi_accessed") wifi_accessed = value.toInt();
      }
    }
    file.close();
  }
...
void handleRoot() {
  ...
  html += "<p><a href='/config'>Edit Live Configuration</a></p>";
...
void handleConfigPage() {
  String html = "<html><body><h1>Runtime Configuration</h1>";
  html += "<form action='/update' method='post'>";
  
  html += "Box Number: <input type='text' name='box_n' value='" + box_n + "'><br>";
  html += "Loops/Wake: <input type='number' name='loops' value='" + String(loopsPerWakeUp) + "'><br>";
  html += "Sleep Time (s): <input type='number' name='sleep' value='" + String(secToSleep) + "'><br>";
  html += "Epoch Time: <input type='number' name='epochTime' value='" + String(epochTime) + "'><br>";
  html += "WiFi Check Interval (s): <input type='number' name='wifiTimeSleep' value='" + String(wifiTimeSleep) + "'><br>";
  html += "Deployment Start: <input type='number' name='deploy_start' value='" + String(deploy_start) + "'><br>";
  html += "Deployment End: <input type='number' name='deploy_end' value='" + String(deploy_end) + "'><br>";
  html += "<input type='submit' value='Save & Apply'>";
  html += "</form><a href='/'>Back to Main</a></body></html>";
...
void handleConfigUpdate() {
  // Validate and update values immediately
  if(server.hasArg("box_n")) box_n = server.arg("box_n");
  if(server.hasArg("loops")) loopsPerWakeUp = server.arg("loops").toInt();
  if(server.hasArg("sleep")) secToSleep = server.arg("sleep").toInt();
  if(server.hasArg("epochTime")) epochTime = server.arg("epochTime").toInt();
  if(server.hasArg("sleep")) {
    secToSleep = server.arg("sleep").toInt();
    nextDataTime = epochTime + secToSleep;
  }
  if(server.hasArg("wifiTimeSleep")) wifiTimeSleep = server.arg("wifiTimeSleep").toInt();
  if(server.hasArg("deploy_start")) deploy_start = server.arg("deploy_start").toInt();
  if(server.hasArg("deploy_end")) deploy_end = server.arg("deploy_end").toInt();

  // Update derived values
  data_filename = "data_" + box_n + ".csv";
  motion_filename = "motion_" + box_n + ".bin";
  TIME_TO_SLEEP = secToSleep;

    server.send(200, "text/html", "<html><body>"
                  "<h1>Configuration Updated!</h1>"
                  "<p>Changes applied immediately</p>"
                  "<a href='/config'><button>Edit Again</button></a>"
                  "<a href='/'><button>Back to Files</button></a>"
                  "</body></html>");
  
  saveParameters(); // Save after successful update
}
...
void startAPDataSharing() {
  ...
  server.on("/update", HTTP_POST, handleConfigUpdate);
  server.on("/stop", HTTP_POST, handleStop);
